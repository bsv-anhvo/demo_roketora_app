...
- (void)adjustCameraFPS:(NSNumber *)fps {
  ...
  CGSize targetSize = _previewSize;
  if (targetSize.width <= 0 || targetSize.height <= 0) {
    targetSize = [self getBestVideoSizeAccordingQuality:_recordingQuality];
  }

  CGFloat targetLonger = MAX(targetSize.width, targetSize.height);
  CGFloat targetShorter = MIN(targetSize.width, targetSize.height);
  ...
  for (AVCaptureDeviceFormat *format in _captureDevice.formats) {
    ...
    CGFloat longer = MAX(dims.width, dims.height);
    CGFloat shorter = MIN(dims.width, dims.height);
    CGFloat area = longer * shorter;
    ...
    if (largeEnough && area < bestLargeArea) {
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection captureVideoOutput:(AVCaptureVideoDataOutput *)captureVideoOutput {
  ...
  if (output == captureVideoOutput) {
    ...
    // custom code - drop frame when writer is busy to avoid NSInternalInconsistencyException
    if (_videoWriterInput.readyForMoreMediaData && nextBuffer != NULL) {
...
- (CGSize)getBestVideoSizeAccordingQuality:(VideoRecordingQuality)quality {
  ...
  // custom code - do not clamp writer size down to preview size
  return size;
}
